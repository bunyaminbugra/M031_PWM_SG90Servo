...
uint32_t PWM_ConfigCaptureChannel(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32UnitTimeNsec, uint32_t u32CaptureEdge)
{
    ...
    if (pwm == ((PWM_T *) ((( uint32_t)0x40000000) + 0x58000)))
        u32Src = ((CLK_T *) ((( uint32_t)0x40000000) + 0x00200))->CLKSEL2 & (0x1ul << (0));
    else
        u32Src = ((CLK_T *) ((( uint32_t)0x40000000) + 0x00200))->CLKSEL2 & (0x1ul << (1));
    ...
    ((pwm)->CLKPSC[(u32ChannelNum) >> 1] = ((uint32_t)u16Prescale));
...
void PWM_EnableADCTrigger(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Condition)
{
    ...
    {
        (pwm)->ADCTS0 &= ~(((0xful << (0))) << (u32ChannelNum << 3UL));
        (pwm)->ADCTS0 |= (((0x1ul << (7)) | u32Condition) << (u32ChannelNum << 3UL));
    }
    else
    {
        (pwm)->ADCTS1 &= ~(((0xful << (0))) << ((u32ChannelNum - 4UL) << 3UL));
        (pwm)->ADCTS1 |= (((0x1ul << (7)) | u32Condition) << ((u32ChannelNum - 4UL) << 3UL));
    }
}
...
void PWM_DisableADCTrigger(PWM_T *pwm, uint32_t u32ChannelNum)
{
    if (u32ChannelNum < 4UL)
    {
        (pwm)->ADCTS0 &= ~((0x1ul << (7)) << (u32ChannelNum << 3UL));
    }
    else
    {
        (pwm)->ADCTS1 &= ~((0x1ul << (7)) << ((u32ChannelNum - 4UL) << 3UL));
    }
}
